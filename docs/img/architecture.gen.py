...
Ажиллуулах:
    ...
    #          --window-size=1200,1165 --force-device-scale-factor=2 architecture.en.html
...
T = {
 "en": dict(
  ...
  core=[("Normalize","raw → finding","",""),
        ("Pod → controller","rollup before dedup · --no-rollup","nw","new"),
 ...
 "mn": dict(
  ...
  core=[("Normalize","raw → finding","",""),
        ("Pod → controller","dedup-аас өмнө rollup · --no-rollup","nw","шинэ"),
...
<div class="band"><div class="lbl">{x['s_core']}</div><div class="row c5">{core}</div></div>{A}
